...
void test1b(void) {
	...
	ErrorExpected(Z502_REG9, "CREATE_PROCESS");        //上面执行完os的context超过27（30）就报错了，下面无法执行了
...
void test1x(void) {
	...
	for (Iterations = 0; Iterations < NUMBER_OF_TEST1X_ITERATIONS; Iterations++) {
...
void test2f(void) {
	...
	for (Iterations = 0; Iterations < NUMBER_OF_ITERATIONS; Iterations++) {
		...
		for (Loops = 0; Loops < LOOP_COUNT; Loops++) {
			...
			//mtr->page_touched[(short) Loops] = (short)Z502_REG7; //modify at 9/13/2013, its worning cant convert 32 to 16
		}   // End of for Loops
